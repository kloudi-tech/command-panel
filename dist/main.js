(function (global, factory) {
  ...
  var UrlUtil = /*#__PURE__*/function () {
    function UrlUtil() {
      classCallCheck(this, UrlUtil);
    }

    createClass(UrlUtil, null, [{
      key: "getCompleteURLFromQueryData",
      value: function getCompleteURLFromQueryData(to) {
        ...
        try {
          ...
          var pathname = "/".concat(to.split("?")[0], "?q=").concat(query);
          url = new URL("".concat(window.location.origin).concat(pathname));
        } finally {
          /*
          ...
          if (url.pathname === "/query/submit/") {
            ...
            var currentURLSegments = window.location.pathname.replace(/(^\/+|\/+$)/g, "").split("/");
            var redirectURLSegments = url.pathname.replace(/(^\/+|\/+$)/g, "").split("/");

            if (currentURLSegments.length >= 4 && redirectURLSegments.length >= 4 && currentURLSegments[0] === redirectURLSegments[0]
            /* Same Project */
            && currentURLSegments[1] === redirectURLSegments[1]
            /* Same Project ID */
            && currentURLSegments[2] === redirectURLSegments[2]
            /* Same Entity */
            && currentURLSegments[3] !== redirectURLSegments[3]) {
              ...
              state: "INTERNAL-DIFFERENT-ENTITY-REDIRECT"
            };
          } else url.redirect = {
            state: "NO-REDIRECT"
  ...
  var useSubmitQuery = function useSubmitQuery(props) {
    ...
    var execute = React.useCallback(function (callbackPayload) {
      ...
      if (cachedData !== null && cachedData !== undefined && Object.keys(cachedData).length > 0) {
        var to = cachedData.navigateTo,
            _query = cachedData.query.text;
  ...
  function ModalCommandPanel(props) {
    ...
    React.useEffect(function () {
      if (["SUCCESS", "ERROR", "STALE-WHILE-REVALIDATE"].indexOf(status) >= 0) {
        setOpen(false);
        if (response && Object.keys(response).length > 0 && response.cards && Object.keys(response.cards).length > 0 && data && Object.keys(data).length > 0 && data.cards && Object.keys(data.cards).length > 0 && JSON.stringify(response.cards) !== JSON.stringify(data.cards)) setResponse(response);
